...
def setup_open_telemetry(settings: AppSettings) -> None:
    """Configure tracing and optional OTLP log export."""
...
def _build_resource(settings: AppSettings):
...
def _configure_trace_export(settings: AppSettings, resource: object) -> bool:
    try:
        ...
        logger.info(
            "OTEL_TRACES_ENABLED | endpoint=%s | service=%s",
            settings.otel_exporter_otlp_endpoint or "default",
...
def _configure_log_export(settings: AppSettings, resource: object) -> bool:
    try:
        ...
        logger.info(
            "OTEL_LOGS_ENABLED | endpoint=%s | service=%s",
            settings.otel_exporter_otlp_logs_endpoint
            or settings.otel_exporter_otlp_endpoint
            or "default",
            settings.otel_service_name,
...
def start_span(
    ...
    *,
    ...
    try:
        ...
        tracer = trace.get_tracer("rag-react-agent")
        kind_map = {
